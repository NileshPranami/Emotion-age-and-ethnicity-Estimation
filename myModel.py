...
class SeparableConvolutionModel(torch.nn.Module):
    ...
    def __init__(self, dropout=0.7, n_class=7, n_channels=1, n_filters=[64, 128, 256, 512], kernels_size=[3, 3, 3, 3]):
        ...
        super(SeparableConvolutionModel, self).__init__()
        ...
        # 1st block
        self.conv1 = SeparableConvolutionLayer(self.n_channels, self.n_filters[0], self.kernels_size[0])
        self.batchnorm1 = torch.nn.BatchNorm2d(self.n_filters[0])
        self.conv2 = SeparableConvolutionLayer(self.n_filters[0], self.n_filters[0], self.kernels_size[0])
        self.batchnorm2 = torch.nn.BatchNorm2d(self.n_filters[0])

        # 2nd block
        self.conv3 = SeparableConvolutionLayer(self.n_filters[0], self.n_filters[1], self.kernels_size[1])
        self.batchnorm3 = torch.nn.BatchNorm2d(self.n_filters[1])
        self.conv4 = SeparableConvolutionLayer(self.n_filters[1], self.n_filters[1], self.kernels_size[1])
        self.batchnorm4 = torch.nn.BatchNorm2d(self.n_filters[1])

        # 3rd block
        self.conv5 = SeparableConvolutionLayer(self.n_filters[1], self.n_filters[2], self.kernels_size[2])
        self.batchnorm5 = torch.nn.BatchNorm2d(self.n_filters[2])
        self.conv6 = SeparableConvolutionLayer(self.n_filters[2], self.n_filters[2], self.kernels_size[2])
        self.batchnorm6 = torch.nn.BatchNorm2d(self.n_filters[2])

        # 4th block
        self.conv7 = SeparableConvolutionLayer(self.n_filters[2], self.n_filters[3], self.kernels_size[3])
        self.batchnorm7 = torch.nn.BatchNorm2d(self.n_filters[3])
        self.conv8 = SeparableConvolutionLayer(self.n_filters[3], self.n_filters[3], self.kernels_size[3])
...
class MultiSeparableConvolutionModel(nn.Module):
    """CNN based on depthwise separable convolution layer Multimodel implementation """
    def __init__(self, dropout=0.7, n_class=[1, 2, 5], n_channels=3,
                 n_filters=[64, 128, 256, 512], kernels_size=[3, 3, 3, 3]):

        super(MultiSeparableConvolutionModel, self).__init__()

        self.n_class = n_class
        self.conv_base = SeparableConvolutionModel(dropout=dropout, n_class=1, n_channels=n_channels,
